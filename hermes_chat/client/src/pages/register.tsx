import React, {useState} from 'react'
import Head from 'next/head'
import Link from 'next/link'
import Image from 'next/image'
import { Layout } from '../layout/layout'
import { BsPersonCircle } from "react-icons/bs";

import styles from '../../styles/Form.module.css'
import white_hermes_sandal from '../../public/assets/white_hermes_sandal.png'

export default function register() {

    return (
        <Layout>
            <Head>
                <title>Hermes Chat - Register</title>
                <meta name="description" content="Generated by create next app" />
                <meta name="viewport" content="width=device-width, initial-scale=1" />
                <link rel="icon" href="/assets/hermes_logo.png" />
            </Head>

            <section className='w-3/4 h-full  mx-auto flex flex-col justify-around sm:justify-between py-5'>
                {/* Logo */}
                <div className='flex justify-center items-center'>
                    <Image src={white_hermes_sandal} alt='logo' height={50}/>
                    <h1 className='text-white text-3xl sm:text-2xl'>Hermes Chat</h1>
                </div>
                {/* Title */}
                <div>
                    <h1 className='text-white text-5xl sm:text-3xl'>Welcome Traveler</h1>
                    <h2 className='text-[#b5bbb3]'>Ready to Register</h2>
                </div>
                {/* Form */}
                <form className='flex flex-col items-center gap-10' onSubmit={(e) => e.preventDefault()}>
                    {/* Info Input */}
                    <div className='w-full sm:w-4/5 flex flex-col gap-5'>
                        <div className={styles.input_group}>
                            <label className='text-white'>Username <span className='text-red-700'>*</span></label>
                            <input className={styles.input_text} type='text' />
                        </div>
                        <div className={styles.input_group}>
                            <label className='text-white'>Password <span className='text-red-700'>*</span></label>
                            <input className={styles.input_text} type='text' />
                        </div>
                    </div>
                    {/* Register Button */}
                    <div className={styles.button}>
                        <button type='submit'>Register</button>
                    </div>
                </form>
                {/* Register */}
                <p className='block text-white'>Already have an account? <Link href={'/login'} className='text-[#2b78cd] hover:underline'>Login</Link></p>
            </section>

        </Layout>
    ) 
}